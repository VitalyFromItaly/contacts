<template>
  <div id="info-details">
  <Modal @close="isEditClicked = !isEditClicked"
        v-if="isEditClicked"
        class="flex items-center justify-center ">
        <template v-slot:header><span class="text-base sm:text-lg">Add a new field</span></template>
        <template v-slot:body>
          <div class=" mx-5">
            <input id="field" v-model="field" :disabled="field == 'name' || field == 'surname'"
            @keydown.enter.prevent="keyAndValueChange" class="w-full text-gray-900 rounded-md pl-3 mt-5"/>
            <label v-if="(isEditClicked && nameField == 'surname') || (isEditClicked && nameField == 'name')"
              class="text-sm uppercase font-semibold text-yellow-400"
              for="field">
              The field can not be editted.
            </label>
            <input
              v-model="value"
              class=" text-gray-900 my-5 rounded-md pl-3 w-full"
              @keydown.enter.prevent="keyAndValueChange"
            />
          </div>
          <button class="bg-green-500 py-1 px-2 rounded-md text-gray-100 hover:bg-green-700 hover:text-white mb-3 mr-5 float-right"
            v-if="isEditClicked"
            @click="keyAndValueChange">
            change
          </button>
      </template>
    </Modal>
    <div class="flex items-end justify-between space-y-4 space-x-5">
      <div class="sm:flex" v-if="!isId">
        <p class="text-gray-500">
          {{ nameField }}:
        </p>
        <p class="ml-1" >{{ nameValue }}</p>
      </div>
      <div v-if="!isId" class="ml-auto">
        ...
        <button v-if="field !== 'name'" @click="deleteKey" class="ml-5">
...
export default {
  ...
  methods: {
    ...mapActions(["DEFINE_CHANGING_KEY_IN_CURRENT_CONTACT", "DEFINE_DELETING_KEY"]),
    editField() {
