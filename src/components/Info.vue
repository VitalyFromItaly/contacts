...
<div id="info">
  ...
  <div v-if="currentContact !== undefined" class="mt-10 text-2xl">
    <InfoDetails v-for="k in keys" :key="k" :k="k" />
    <!--<InfoDetails v-for="(value,name) in GET_CURRENT_CONTACT" :key="name" :name="name" :value="value"  />-->
    <!--
      <div  v-for="key in keys" :key="key" class="flex">
      <p class="text-gray-400">{{key}} :</p>
      <p  v-if="!isEditClicked" class=""> {{currentContact[key] }}</p>
      <input v-model="currentContact[key]"  v-if='isEditClicked' >
      <input v-model="key.name"  v-if='isEditClicked' >
    ...
    <div v-if="isAddFieldClicked">
      ...
      <button @click="submitAdding">submit</button>
    </div>
    <button class="bg-blue-400 text-gray-50 py-1 px-2 rounded-md my-10" @click="isAddFieldClicked = !isAddFieldClicked">add field</button>
  </div>
...
export default {
  ...
  methods: {
    ...mapActions(['DEFINE_CURRENT_CONTACT']),

    editField() {
      this.isEditClicked = !this.isEditClicked
      console.log(this.GET_CURRENT_CONTACT);
    },
    addField() {
      this.isAddFieldClicked = true;
    },
    submitAdding() {
      this.isAddFieldClicked = !this.isAddFieldClicked;
      this.keys.push(this.newFieldName);
      this.GET_CURRENT_CONTACT[this.newFieldName] = this.newFieldValue;
    },
    setCurrentContact() {
